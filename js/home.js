...
//homepage testimonails swiper
var swiper = new Swiper(".testimoSwiper", {
  slidesPerView: 1,
  spaceBetween: 10,
  freeMode: true,
  pagination: {
...
//homepage video tabs horizontal video swiper
...
//services tabs videos play and pause
const muteVideobyBtn = async (videoId, playBtnId) => {
  let video = document.getElementById(videoId);
  console.log('video ', video);
  let allvideo = document.querySelectorAll('.videos-block video');
  let mutebtn = document.querySelectorAll('.mute-video');
  mutebtn = [...mutebtn].filter((btn) => btn.id !== playBtnId)
  let vdos = [...allvideo];
  vdos = vdos.filter((vdo) => vdo.id !== videoId)
  let playBtn = document.getElementById(playBtnId);
  ...
  let icon = document.querySelector(`#${playBtnId} > i`);
  if (video.muted == true) {
...
document.addEventListener('readystatechange', async (event) => {
  ...
  if (event.target.readyState === "complete") {
    ...
    if (window.location.href.includes("video-ads")) {
      ...
      await resolveImages(videos, "homegif")
...
function sendEmail() {
  ...
  // const endpoint = 'http://localhost:3100/v1/user/sendEmail';
  const endpoint = 'https://myserver.mywalll.com/v1/user/sendEmail'; // Use this for production
